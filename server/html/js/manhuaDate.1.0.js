...
$(function() {
	...
	$.fn.manhuaDate = function(options) {
			...
			if (dv != "&nbsp;"){
				 ...
				 if (options.isTime){			
					...
					/*这里比较然后改变str					 
						...
						var yourdate = new Date($("#year").val(),$("#month").val()-1,dv);
						
						//alert (nowdate+''+yourdate);
						
						
						if (Date.parse(yourdate)>=Date.parse(nowdate))	 
							{
							  if ($("#month").val().length == 1){ var monthtxt="0"+$("#month").val(); } else { var monthtxt=$("#month").val(); }
							  if (dv.length == 1){ var daytxt="0"+dv; } else {var daytxt=dv;}
							  str = $("#year").val()+ options.fuhao +monthtxt+ options.fuhao +daytxt;
							}
						else
						    {
		...
		$(document).click(function(e){
			  if($(e.target).is(".calender,.calender *,#date")){
			   return false;
			  }
			  $(".calender").hide();
		});
